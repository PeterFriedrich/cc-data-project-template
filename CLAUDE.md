...
- **No scheduled PR check-ins.** Each wake is a full turn of usage, and a PR waiting overnight on the owner's merge gains nothing from one. Subscribe to PR activity so CI and reviews still arrive; never arm `send_later` or a trigger for a PR. Details: `.claude/skills/steward/SKILL.md`.
